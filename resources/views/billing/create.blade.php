<x-app-layout>
    <x-slot name="header">
        <x-page-header title="Record Bill" subtitle="{{ $patient->name }} — {{ $patient->type->label() }}">
            <x-slot name="actions">
    ...
    <div class="mb-6 grid gap-4 lg:grid-cols-3">
        <div @class([
            ...
            <p class="section-subtitle">Available Balance</p>
            <p class="mt-1 text-2xl font-bold text-slate-900">K {{ number_format($availableBalance, 2) }}</p>
            <p class="mt-1 text-sm text-slate-600">Payer: {{ $patient->effectiveBalanceOwnerLabel() }}</p>
            @if ($availableBalance < $lowBalanceThreshold)
                <p class="mt-2 text-sm font-medium text-amber-800">
                    <i class="fa-solid fa-triangle-exclamation"></i> Low balance warning
                </p>
            @endif
            @if ($patient->isDependant() && $patient->principalMember)
                <p class="form-hint mt-1">Patient: {{ $patient->name }} ({{ $patient->relationship }})</p>
            @endif
        </div>
        ...
        <div class="card card-body lg:col-span-2">
            ...
            <dl class="mt-4 grid gap-3 sm:grid-cols-2 text-sm">
                <div>
                    <dt class="text-slate-500">HC Number</dt>
    ...
    <div class="card card-body max-w-3xl">
        <form method="POST" action="{{ route('billing.store') }}"
            ...
            <div>
                <h3 class="section-title mb-3">Itemised Charges (K)</h3>
                <div class="grid gap-4 sm:grid-cols-2">
                    @foreach ([
            ...
            <div class="card card-body bg-slate-50 space-y-2">
                <div class="flex justify-between text-sm">
                    <span class="font-medium text-slate-700">Bill Total</span>
                    <span class="font-bold text-slate-900" x-text="'K ' + total.toFixed(2)"></span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-slate-600">Balance after bill</span>
                    <span class="font-medium" :class="isLowAfter ? 'text-amber-700' : 'text-slate-900'"
            ...
            <div class="panel-footer -mx-6 -mb-6 mt-6 flex items-center gap-3 px-6 py-4">
                <button type="submit" class="btn-primary">
                    <i class="fa-solid fa-file-invoice-dollar"></i> Post Bill &amp; Print Receipt
                </button>
            </div>
        </form>
    </div>
